...
|Seek back 10 minutes|GET | link:http://localhost:8080/api/seek-back/PT10M[role=resource,window=_blank]
|Seek back to the beginning|GET | link:http://localhost:8080/api/seek-to-beginning[role=resource,window=_blank]
|Seek back to the end|GET | link:http://localhost:8080/api/seek-to-end[role=resource,window=_blank]
|===
...
....
http --json POST http://localhost:8080/api/schema/user1/to-avro-hex/ \
    name="Maria do Carmo Mão de Ferro e Cunha de Almeida Santa Rita Santos Abreu" age:=33
....
...
curl -X 'POST' 'http://localhost:8080/avro/schema/user1/hexdump/' \
  ...
  -d '{"name":"Maria do Carmo Mão de Ferro e Cunha de Almeida Santa Rita Santos Abreu","age":33}'
....
